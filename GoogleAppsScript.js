...
function saveSystemConfig(data) {
  ...
  try {
    ...
    const configPairs = {
      'easyecom_url': data.easyecom_url,
      'easyecom_token': data.easyecom_token,
      'easyecom_email': data.easyecom_email,
      'nimbus_notification_email': data.nimbus_notification_email
    };
    
    Object.keys(configPairs).forEach(key => {
      const val = configPairs[key];
    ...
    Logger.log("Failed to save to System_Config sheet: " + e.toString());
  }
...
function getSystemConfig() {
  try {
    ...
    for (let i = 1; i < dataRange.length; i++) {
      if (dataRange[i][0]) {
        configData[dataRange[i][0]] = dataRange[i][1];
      }
    }
    return responseJSON({status: 'success', data: configData});
  } catch (err) {
    return responseJSON({status: 'error', message: err.toString()});
  }
}
